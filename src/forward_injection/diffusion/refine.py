...
def refine_image(refined_prompt, init_latents, start_timestep=25, save_intermediate_steps=False, refinement_step=None, adjusted_refinement_step=None, output_dir=None, prefix="", model_version="", n_offset_latent_step=None):
    """
    ...
    for step, t in enumerate(tqdm(remaining_timesteps, desc="Refining")):
        ...
        # Compute offset for the next restart step
        # if next_restart_step is not None:
        #     current_remaining_steps = 100 - start_timestep
        #     offset_latent_step = math.ceil((next_restart_step * current_remaining_steps) / 100)
        # else:
        #     offset_latent_step = None  # No further restart step

        # Save latent at the offset step for the next refinement iteration
        if step == n_offset_latent_step:
            latents_dict[step] = latents.clone()
            print(f"✅ Saved latent state at offset step {step} in memory for next refinement.")
    ...
    return final_image, latents_dict
