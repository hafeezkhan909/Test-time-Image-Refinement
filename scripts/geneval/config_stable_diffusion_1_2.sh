#!/bin/bash
# Environment and pipeline configuration, shared across run_*.sh scripts
...
MODEL_VERSION="1.5"                                # Stable Diffusion version: 1.4, 1.5, or 2.1
...
SELECTION_MODE="first_early_stop"                  # Which image counts as "final" per prompt when post-processing: first_early_stop (ES1->ES2->ES3->last round) or final_only (always the last round)

# Output locations
OUTPUT_DIR="new_outputs/batch_results"             # Where main.py writes raw per-prompt outputs
